...
app.controller("PumpController", ["$rootScope", "$scope", "$aside", "$http", "$location", "$timeout", "Upload", "layerService", function(rootScope, scope, aside, http, location, timeout, upload, layerService) {
    ...
    createCollectDeviceForm = aside({
        scope: scope,
        template: "tpl/form-collect-device-create.jspx",
        show: !1,
        placement: "left",
        backdrop: true
    }),
    updateCollectDeviceForm = aside({
        scope: scope,
        template: "tpl/form-collect-device-update.jspx",
        show: !1,
        placement: "left",
        backdrop: true
    }),
		...
		callback : {
			onClick: function(event, treeId, treeNode) {
				if(!treeNode.isParent && !loading) {
					...
					if(scope.pumpd && scope.pumpd.base && scope.pumpd.base.pumpId && treeNode.id == scope.pumpd.base.pumpId) {
						return;
					}
	...
	function initMoreLineOption(data) {
		return {
			    tooltip: {
			        trigger: 'axis',
			        formatter: function(value){
			        	return '采集项：' + value.seriesName + "<br/>采集值：" + value.data[1].toFixed(2) + '<br/>采集时间：' + new Date(value.data[0]).format("hh:mm:ss");
			        },
			        axisPointer: {
			            type: 'cross',
			            label: {
			    ...
			    legend: {
			        data:['设置温度','环境温度','室内温度','水箱温度','出水温度','回水温度','排气温度','回气温度','压缩机电流']
			    },
			    grid: {
			        x: 10,
			        y: 70,
			    ...
			    series: [
			        ...
			        },
			        {
			            ...
			            yAxisIndex: 1,
			            smooth: true,
			            data:data.compressorCurrents
			        }
			    ]
			}
	}
	
	function loadPumpHistoryData(pumpId, date){
		var historyChart = echarts.init(document.getElementById("history_echart_box"));
		http.get("monitor/pump-history-data.jspx?pumpId=" + pumpId + "&date=" + date).success(function (response) {
			if(response.success) {
				historyChart.setOption(initMoreLineOption(response), true);
			} else {
				layerService.layerAlert(response.message)
			}
		})
	...
	scope.searchPumpHistoryData = function(){
		loadPumpHistoryData(consoleCurrentPumpId, scope.historyDate.format('yyyy-MM-dd'));
	}
	...
	scope.uploadPumpImg = function ($file, pumpId) {
    	if (!$file) {
    		return;
    	}
    	...
    	if($file.size > 4 * 1024 * 1024){
    		layerService.layerAlert("文件大小超过4Mb限制");
    		return;
    	}
        upload.upload({
            url: 'file/upload-pump-img.jspx',
            data: {file: $file, pumpId: pumpId}
        }).then(function (resp) {
        	layerService.layerAlert(resp.data.message);
            if(resp.data.success) {
            	scope.pictures.unshift(resp.data.res);
            	if(resp.data.pump) {
					scope.pumpd.base = resp.data.pump
				}
            }
        	...
        	if(scope.progressPercentage == 100) {
        		scope.cMessage = "上传成功，正在压缩中...";
        		timeout(function(){
        			scope.cMessage = "为了访问效果更好，文件还在压缩中，请耐心等待...";
        			timeout(function(){
            			scope.cMessage = "这次时间有点长，可能文件比较大...";
            			timeout(function(){
                			scope.cMessage = "压缩还在继续，实在是不给力呀...";
                			timeout(function(){
                    			scope.cMessage = "你可以终止压缩啦！";
                    		}, 20*1000);
                		}, 10*1000);
            		}, 10*1000);
        		}, 5*1000);
    ...
    scope.deletePumpImg = function (res, pumpId) {
    	res && pumpId && layerService.layerConfirm("确认要删除该热泵图片吗？", function(){
    		http.get("file/delete-pump-img.jspx?resId=" + res.resId + "&pumpId=" + pumpId).success(function (response) {
    			layerService.layerAlert(response.message)
				if(response.success) {
					scope.pictures.splice(scope.pictures.indexOf(res), 1);
					if(response.pump) {
						scope.pumpd.base = response.pump
					}
				}
    ...
    scope.setPumpDefaultImg = function (resId, pumpId) {
    	resId && pumpId && layerService.layerConfirm("确认要将该图片设置为热泵的默认图片吗？", function(){
    		http.get("file/set-pump-default-img.jspx?resId=" + resId + "&pumpId=" + pumpId).success(function (response) {
    			layerService.layerAlert(response.message)
    			if(response.success && response.pump) {
    				scope.pumpd.base = response.pump
    			}
    		})
    	})
	...
	scope.createCollectDevice = function(pump){
		if(pump){
			http.get("manage/device-product-data.jspx").success(function(response) {
	        	scope.products = response.Cs;
				scope.device = {};
				createCollectDeviceForm.show()
			});
		} 
	},
	
	scope.saveCollectDevice = function(){
		scope.device.pumpId = scope.pumpd.base.pumpId;
		if(scope.products && scope.products.selected) {
			scope.device.deviceProductId = scope.products.selected.productId;
		}
		http({
    		method: 'post',
    		url: 'monitor/save-collect-device.jspx',
    		data: scope.device
    	}).success(function(response) {
			layerService.layerAlert(response.message)
    		if(response.success) {
    			scope.pumpd.devices.push(response.device)
    			createCollectDeviceForm.hide()
    		}
    	})
	},

	scope.updateCollectDevice = function(device){
		if(device){
			http.get("manage/device-product-data.jspx").success(function(response) {
	        	scope.products = response.Cs;
	        	angular.forEach(scope.products, function(produce) {
	                if(produce.productId == device.deviceProductId) {
	                	scope.products.selected = produce;
	                }
	            });
				scope.device = angular.copy(device);
				updateCollectDeviceForm.show()
			});
		} 
	},
	
	scope.changeCollectDevice = function(){
		if(scope.products && scope.products.selected) {
			scope.device.deviceProductId = scope.products.selected.productId;
		} else {
			scope.device.deviceProductId = ''
		}
		console.info(scope.device)
		http({
    		method: 'post',
    		url: 'monitor/update-collect-device.jspx',
    		data: scope.device
    	}).success(function(response) {
			layerService.layerAlert(response.message)
    		if(response.success) {
    			scope.pumpd.devices.forEach(function(device,index,arr){
  				  if(device.deviceId == response.device.deviceId) {
  					  arr[index] = response.device;
  				  }
  				}),
    			updateCollectDeviceForm.hide()
    		}
    	})
	}
	
	scope.removeCollectDevice = function(device){
		device && layerService.layerConfirm("确认要移除该采集设备吗？", function(){
			http({
	    		method: 'post',
	    		url: 'monitor/remove-collect-device.jspx',
	    		data: {
				...
				layerService.layerAlert(response.message)
	    		if(response.success) {
	    			scope.pumpd.devices.splice(scope.pumpd.devices.indexOf(device), 1),
	    			updateCollectDeviceForm.hide()
	    		}
	    	})
		})
        		...
        		timeout(function(){
        			...
        			timeout(function(){
            			...
            			timeout(function(){
                			scope.cMessage = "压缩还在继续，实在是不给力呀...";
                			timeout(function(){
                    			scope.cMessage = "已上传成功，但未完成压缩，你可以终止压缩啦！";
                    		}, 20*1000);
                		}, 10*1000);
            		}, 10*1000);
        		}, 5*1000);
...
app.controller("ConservationController", [ function() {}]),
...
app.controller("RepairController", ["$scope", "$http", "$aside", "layerService", "$timeout", "Upload", function(scope, http, aside, layerService, timeout, upload) {
	var createForm = aside({
	    scope: scope,
	    template: "tpl/form-repair-create.jspx",
        show: !1,
        placement: "left",
        backdrop: true
	}),
	selectPumpForm = aside({
	    scope: scope,
	    template: "tpl/form-select-pump.jspx",
	    animation: "am-fade-and-slide-top",
	    show: !1,
	    backdrop: true
	}),
	detailForm = aside({
	    scope: scope,
	    template: "tpl/form-repair-detail.jspx",
        show: !1,
        placement: "left",
        backdrop: true
	}),
	evaluateForm = aside({
	    scope: scope,
	    template: "tpl/form-repair-evaluate.jspx",
	    animation: "am-fade-and-slide-top",
	    show: !1,
	    backdrop: true
	});
	
	http.get("stoppage/my-data.jspx").success(function (response) {
		if(response.success) {
			scope.data = response.data;
		} else {
			layerService.layerAlert(response.message)
	...
	scope.create = function(){
		scope.currentPumpId = '';
		scope.currentPumpText = '';
		http.get("get-group-id.jspx").success(function (response) {
			scope.currentGroupId = response.groupId;
			scope.files = [];
			scope.stoppage = {
			    descript:'',
			    occurTime1:'',
	...
	scope.save = function(){
		if(!scope.currentPumpId) {
			layerService.layerAlert("请选择一个热泵");
			return;
		}
		if(!scope.currentGroupId) {
			layerService.layerAlert("无效的表单信息");
			return;
		}
		if(!scope.stoppage.descript || scope.stoppage.descript=='') {
			layerService.layerAlert("请输入故障描述");
			return;
		}
		http({
    		method: 'post',
    		url: 'stoppage/save.jspx',
    		data: {
    			pumpId : scope.currentPumpId,
    			occurDate : scope.stoppage.occurTime1,
    			occurTime : scope.stoppage.occurTime2,
    			descript : scope.stoppage.descript,
    			attachId : scope.currentGroupId
    		}
    	}).success(function(response) {
			layerService.layerAlert(response.message)
    		if(response.success) {
    			scope.data.unshift(response.stoppage);
    			createForm.hide();
    		}
    	})
	}
	
	scope.detail = function(stoppageId){
		stoppageId && http.get("stoppage/detail.jspx?stoppageId=" + stoppageId).success(function (response) {
			scope.stoppaged = {
				stoppage : response.stoppage,
				pump : response.pump,
				reses : response.reses,
				handles : response.handles,
				pumpWarn : response.pumpWarn
			};
			detailForm.show();
		})
	}

	scope.remove = function(stoppageId){
		stoppageId && layerService.layerConfirm("确认要删除该故障和相关处理记录吗？", function(){
			http.get("stoppage/delete.jspx?stoppageId=" + stoppageId).success(function (response) {
				layerService.layerAlert(response.message);
				if(response.success) {
					detailForm.hide();
					http.get("stoppage/my-data.jspx").success(function (response) {
	...
	scope.selectRepairPump = function(){
		selectPumpForm.show();
		var setting = {
			async: {
				enable: true,
			...
			callback : {
				onClick: function(event, treeId, treeNode) {
					if(!treeNode.isParent) {
						scope.currentPumpId = treeNode.id;
						scope.currentPumpText = treeNode.name;
						selectPumpForm.hide();
						scope.$apply();
					}
				}
		...
		timeout(function(){
			$.fn.zTree.init($("#selectPumpTree"), setting);
		})
	}
	
	scope.uploadFile = function ($file, groupId) {
    	if (!$file) {
    		return;
    	}
		if(scope.files.length>(5-1)) {
			layerService.layerAlert("最多上传5个附件");
    		return;
		}
    	if($file.size > 4 * 1024 * 1024){
    		layerService.layerAlert("文件大小超过4Mb限制");
        ...
        upload.upload({
            url: 'file/upload-file.jspx',
            data: {file: $file, groupId: groupId}
        }).then(function (resp) {
        	layerService.layerAlert(resp.data.message);
            if(resp.data.success) {
            	scope.files.unshift(resp.data.res);
            }
            scope.progressPercentage = 0;
        }, function (resp) {
        	layerService.layerAlert("操作失败，请联系管理员");
	...
	scope.removeFile = function(res){
		res && layerService.layerConfirm("确认要删除该文件吗？", function(){
    		http.get("file/delete-file.jspx?resId=" + res.resId).success(function (response) {
    			layerService.layerAlert(response.message)
				if(response.success) {
					scope.files.splice(scope.files.indexOf(res), 1);
				}
    		})
    	})
	}
	...
	scope.formatNum = function(num){
		var fl = 1024;
		if(num>fl*fl) {
			return (num/fl/fl).toFixed(2) + " M";
		}
		if(num>fl) {
			return (num/fl).toFixed(2) + " K";
		}
		return num + " B";
	}
	...
	scope.toEvaluate = function(handle){
		if(handle){
			scope.evaluateLevels = [{name:"A（五星）",value:"A"},{name:"B（四星）",value:"B"},{name:"C（三星）",value:"C"},{name:"D（二星）",value:"D"},{name:"E（一星）",value:"E"}];
			scope.currentHandle = handle;
			evaluateForm.show()
		}
	}
	...
	scope.saveEvaluate = function(){
		http({
    		method: 'post',
    		url: 'stoppage/save-evaluate.jspx',
    		data: {
    			stoppageHandleId : scope.currentHandle.stoppageHandleId,
    			evaluateLevel : scope.evaluateLevels.selected.value,
    			evaluateDescript : scope.currentHandle.evaluateDescript
    		}
    	}).success(function(response) {
			layerService.layerAlert(response.message)
    		if(response.success) {
    			scope.stoppaged.handles.forEach(function(handle,index,arr){
				  if(handle.stoppageHandleId == response.handle.stoppageHandleId) {
					  arr[index] = response.handle;
				  }
    			});
    			evaluateForm.hide();
    		}
	...
	scope.formatLevel = function(level){
		if(level == 'A') {
			return '<span style="color:green"><i class="md md-star"></i><i class="md md-star"></i><i class="md md-star"></i><i class="md md-star"></i><i class="md md-star"></i></span>';
		} else if(level == 'B') {
			return '<span style="color:green"><i class="md md-star"></i><i class="md md-star"></i><i class="md md-star"></i><i class="md md-star"></i><i class="md md-star-outline"></i></span>';
		} else if(level == 'C') {
			return '<span style="color:orange"><i class="md md-star"></i><i class="md md-star"></i><i class="md md-star"></i><i class="md md-star-outline"></i><i class="md md-star-outline"></i></span>';
		} else if(level == 'D') {
			return '<span style="color:orange"><i class="md md-star"></i><i class="md md-star"></i><i class="md md-star-outline"></i><i class="md md-star-outline"></i><i class="md md-star-outline"></i></span>';
		} else if(level == 'E') {
			return '<span style="color:red"><i class="md md-star"></i><i class="md md-star-outline"><i class="md md-star-outline"></i><i class="md md-star-outline"></i><i class="md md-star-outline"></i></span>';
		} else {
			return false;
		}
	}
...
app.controller("HandlingController", ["$scope", "$http", "$aside", "layerService", "localStorageService", function(scope, http, aside, layerService, localStorage) {
	...
	function loadNoAllotStoppage(){
		http.get("stoppage/no-allot-stoppage.jspx").success(function (response) {
			scope.noAllotPageData = response.data;
		})
	}
...
app.controller("WeatherWarningController", ["$scope", "$http", "$aside", "layerService", "localStorageService", function(scope, http, aside, layerService, localStorage) {
	...
	scope.types =[{name:'空气重污染'},{name:'台风'},{name:'暴雨'},{name:'暴雪'},{name:'寒潮'},{name:'大风'},{name:'沙尘暴'},{name:'高温'},{name:'干旱'},{name:'雷电'},{name:'冰雹'},{name:'霜冻'},{name:'霾'},{name:'道路结冰'},{name:'寒冷'},{name:'灰霾'},{name:'雷电大风'},{name:'森林火险'},{name:'降温'},{name:'道路冰雪'},{name:'干热风'},{name:'低温'},{name:'冰冻'}];
...
/*
 * <form name="form"> <div class="btn" data-ngf-select data-ng-model="file"
 * data-name="file" data-ngf-max-size="2MB"
 * data-ngf-accept="'image/gif,image/jpeg,image/jpg,image/png'">选择单文件</div>
 * <button type="submit" data-ng-show="form.file.$valid"
 * data-ng-click="uploadImg()">submit</button> </form>
 * 
 * 
 * scope.uploadImg = function () { if (!scope.form.file.$valid || !scope.file) {
 * layerService.layerAlert("无效的文件"); return; }
 * if(scope.file.type.indexOf("image") !== 0){
 * layerService.layerAlert("无效的文件类型"); return; } if(scope.file.size > 4 * 1024 *
 * 1024){ layerService.layerAlert("文件大小超过2M限制"); return; } upload.upload({ url:
 * 'file/upload-img.jspx', data: {file: scope.file, groupId: "groupId"}
 * }).then(function (resp) { console.log(resp.config.data.file);
 * layerService.layerAlert(resp.data.message); if(resp.data.success) {
 * console.info(resp.data.res) } }, function (resp) {
 * layerService.layerAlert("上传失败"); console.log('Error status: ' + resp.status); },
 * function (evt) { console.info(evt) var progressPercentage = parseInt(100.0 *
 * evt.loaded / evt.total); console.log('progress: ' + progressPercentage + '% ' +
 * evt.config.data.file.name); }); };
 * 
 */