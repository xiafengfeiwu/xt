...
app.controller("ProjectController", ["$scope", "$aside", "$http", "layerService", function(scope, aside, http, layerService) {
    ...
    showCreatPumpForm = function() {
        angular.element(".tooltip").remove(),
        creatPumpForm.hide(),
        creatPumpForm.show()
    },
    hideCreatPumpForm = function() {
    	creatPumpForm.hide()
    },
    ...
    scope.hideCreatPumpForm = function() {
    	hideCreatPumpForm()
    },
    ...
    scope.manage = function(project){
    	http.get("manage/project-area-info.jspx?projectAreaId=" + project.projectAreaId).success(function(response) {
    		scope.projectArea = response.data,scope.project = project,showManageForm(),setProjectManageBoxWidth();
        	$(window).on("resize",function(){setProjectManageBoxWidth()});
    	});
    },
    
    scope.createPump = function(project){
    	scope.products = [{productName:"产品1"},{productName:"产品2"}]
    	showCreatPumpForm()
...
app.controller("DeviceController", ["$scope", "$http", "$aside", "layerService", function(scope, http, aside, layerService) {
	
}]),

app.controller("WarnController", ["$scope", "$http", "$aside", "layerService", function(scope, http, aside, layerService) {
	
}]),

app.controller("ReportController", ["$scope", "$http", "$aside", "layerService", function(scope, http, aside, layerService) {
	
}]),

app.controller("ConsumptionController", ["$scope", "$http", "$aside", "layerService", function(scope, http, aside, layerService) {
	scope.tabFlag = 'hour',
